...
const Schema = require('waterline-schema');
const buildDatastoreMap = require('./waterline/utils/system/datastore-builder');
const buildLiveWLModel = require('./waterline/utils/system/collection-builder');
const BaseMetaModel = require('./waterline/MetaModel');
const getModel = require('./waterline/utils/ontology/get-model');
...
export default class Orm {
  ...
  public Model = BaseMetaModel;

  // Expose `Collection` as an alias for `Model`, but only for backwards compatibility.
  public Collection = BaseMetaModel;
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  loadCollection() {
    ...
    this.registerModel.apply(this, Array.prototype.slice.call(arguments));
  };
  ...
  initialize(options) {
    ...
    try {
      ...
      if (_.keys(this.modelMap).length) {
      ...
      try {
        this.datastoreMap = buildDatastoreMap(options.adapters, options.datastores);
      } catch (e) { throw e; }

      // Build a schema map
      var internalSchema;
      try {
        internalSchema = new Schema(this.modelDefs, options.defaults);
      } catch (e) { throw e; }
      ...
      _.each(internalSchema, function (val, table) {
        ...
        this.modelDefs.push(BaseMetaModel.extend(internalSchema[table]));
      });


      // Hydrate each model definition (in-place), and also set up a
      // reference to it in the model map.
      _.each(this.modelDefs, function (modelDef) {
        ...
        var WLModel = buildLiveWLModel(modelDef, this.datastoreMap, this.context);

        // Store the live Waterline model so it can be used
        // internally to create other records
        this.modelMap[WLModel.identity] = WLModel;
    ...
    async.each(_.keys(this.datastoreMap), (datastoreName, next) => {

      var datastore = this.datastoreMap[datastoreName];
      ...
      try {
        ...
        _.each(modelIdentities, function (modelIdentity) {
          var WLModel = this.modelMap[modelIdentity];
      ...
      return {
        collections: this.modelMap,
        datastores: this.datastoreMap
      }
  ...
  public async teardown() {
    ...
    async.each(_.keys(this.datastoreMap), function (datastoreName, next) {
      var datastore = this.datastoreMap[datastoreName];
  ...
  public async start(options, done) {
    ...
    try {
      ...
      _.each(options.models, function (userlandModelDef, key) {
        ...
        orm.registerModel(this.Model.extend(userlandModelDef));

      });//</_.each>


      // Fire 'er up
      let _classicOntology:any = await orm.initialize({
   ...
   * ------------------------------------------------------------------------------------------
   */
  getModel(modelIdentity: any, orm: any) {
